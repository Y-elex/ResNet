...
def create_data_loaders(train_dir: str, valid_dir: str, batch_size: int):
    ...
    transform = transforms.Compose([
        transforms.Resize((224, 224)),  # 假设输入图像尺寸为224x224
        transforms.ToTensor(),
...
def _resnet(c: Configs):
    ...
    classification = nn.Linear(c.n_channels[-1], 7)  # 输出7个类别，假设有7个情感类别
...
def main():
    ...
    # 设置数据集路径
    train_dir = r'E:\python code\annotated_deep_learning_paper_implementations\Imagenet32_Scripts\data\CK+\train'
    valid_dir = r'E:\python code\annotated_deep_learning_paper_implementations\Imagenet32_Scripts\data\CK+\val'
    ...
    for epoch in range(conf.epochs):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), conf.pretrained_model_path)  # 保存最佳模型
